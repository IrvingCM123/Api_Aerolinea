...
export function registrarAviones(fabricantes: any, modelos: any) {
  ...
  const tipos_Motores = [TipoMotores.TURBINA, TipoMotores.HELICE, TipoMotores.REACCION, TipoMotores.PISTONES];

  const estado_Operativo = [ESTADO_OPERATIVO.OPERATIVO, ESTADO_OPERATIVO.MANTENIMIENTO, ESTADO_OPERATIVO.DESCOMPUESTO, ESTADO_OPERATIVO.FUERA_DE_SERVICIO, ESTADO_OPERATIVO.NO_DISPONIBLE];

  const estado_Logico = [Estado_Logico.ACTIVO, Estado_Logico.INACTIVO];

  const autonomia = ['1000 km', '1200 km', '1100 km', '1050 km', '1150 km', '1080 km', '1100 km', '1150 km', '1120 km', '1200 km'];

  const fabricantes_array: [] = fabricantes;

  const modelos_array: [] = modelos;

  let aviones_generados = [];

  for (let i = 0; i < 10; i++) {
    const avion: Avion = {
      modeloAvionId: modelos_array[Math.floor(Math.random() * modelos_array.length)],
      fabricanteId: fabricantes_array[Math.floor(Math.random() * fabricantes_array.length)],
      ...
      avion_Estado_Operativo: estado_Operativo[Math.floor(Math.random() * estado_Operativo.length)],
      avion_Estado_Logico: estado_Logico[Math.floor(Math.random() * estado_Logico.length)],
      avion_Tipo_Motor: tipos_Motores[Math.floor(Math.random() * tipos_Motores.length)],
...
function numero_Random_Con_Limite(limite_maximo: number, limite_minimo: number): number {
  return Math.floor(Math.random() * (limite_maximo - limite_minimo) + limite_minimo);
}
