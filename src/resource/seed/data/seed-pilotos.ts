...
export function registrar_Pilotos() {
  const nombres = ['Juan', 'Pedro', 'Luis', 'Carlos', 'Javier', 'Miguel', 'Angel', 'Jose', 'Manuel', 'Antonio',];
  const apellidos = ['Perez', 'Gonzalez', 'Rodriguez', 'Fernandez', 'Lopez', 'Martinez', 'Sanchez', 'Gomez', 'Diaz', 'Hernandez',];
  const nacionalidades = ['Mexicana', 'Estadounidense', 'Canadiense', 'Española', 'Francesa', 'Alemana', 'Italiana', 'Brasileña', 'Argentina', 'Chilena',];
  ...
  return generar_Pilotos(nombres, apellidos, nacionalidades, certificaciones, estado_Logico);

}

function generar_Pilotos(nombres: string[], apellidos: string[], nacionalidades: string[], certificaciones: string[], estadologico: string[]) {
  let pilotos_generados = [];

  for (let i = 0; i < 10; i++) {
    const piloto = generar_Piloto(nombres[Math.floor(Math.random() * nombres.length)], apellidos[Math.floor(Math.random() * apellidos.length)], nacionalidades[Math.floor(Math.random() * nacionalidades.length)], certificaciones, estadologico[Math.floor(Math.random() * estadologico.length)]);
...
function generar_Piloto(nombre: string, apellido: string, nacionalidad: string, certificaciones: string[], estadologico: string) {
  const piloto: Piloto = {
    ...
    piloto_Fecha_Nacimiento: new Date(generar_año_Fecha_Nacimiento(), Math.floor(Math.random() * 12), Math.floor(Math.random() * 28)),
    piloto_Nacionalidad: nacionalidad,
    piloto_Horas_Vuelo: Math.floor(Math.random() * 2000),
    piloto_Certificaciones: certificaciones.sort(() => Math.random() - Math.random()).slice(0, 4),
    piloto_Fecha_Expedicion_Licencia: new Date(generar_año_Fecha_Expedicion_Licencia(), Math.floor(Math.random() * 12), Math.floor(Math.random() * 28)),
...
function generar_Correos_Electronicos(piloto_Nombre: string, piloto_Apellidos: string) {
  ...
  return nombre_Piloto.toLowerCase().replace(/ /g, '') + correos_Array[Math.floor(Math.random() * correos_Array.length)];
